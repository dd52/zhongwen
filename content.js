  ...
  switch (keyDown.keyCode) {
    ...
    case 66: // 'b'
      {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          if (ret === 0) {
            ...
            savedRangeNode = findPreviousTextNode(
              savedRangeNode.parentNode,
              savedRangeNode
            );
            savedRangeOffset = 0;
    ...
    case 71: // 'g'
      if (config.grammar !== "no" && savedSearchResults.grammar) {
        ...
        let allset =
          "https://resources.allsetlearning.com/chinese/grammar/" + sel;
    ...
    case 78: // 'n'
      for (let i = 0; i < 10; i++) {
        ...
        if (ret === 0) {
          ...
          savedRangeNode = findNextTextNode(
            savedRangeNode.parentNode,
            savedRangeNode
          );
          savedRangeOffset = 0;
...
function makeHtml(result, showToneColors) {
  ...
  html += '<span class="cantonese-header">Mandarin</span><br><br>';

  if (result === null) return "";

  for (let i = 0; i < result.data.length; ++i) {
    entry = result.data[i][0].match(
      /^([^\s]+?)\s+([^\s]+?)\s+\[(.*?)\]?\s*\/(.+)\//
    );
    if (!entry) continue;
    ...
    if (config.simpTrad === "auto") {
      ...
      if (entry[1] !== entry[2]) {
        html +=
          '<span class="' + hanziClass + '">' + entry[1] + "</span>&nbsp;";
      }
    }
    ...
    if (
      config.grammar !== "no" &&
      result.grammar &&
      result.grammar.index === i
    ) {
      html +=
        '<br><span class="grammar">Press "g" for grammar and usage notes.</span><br>';
      addFinalBr = true;
    }
    ...
    if (config.vocab !== "no" && result.vocab && result.vocab.index === i) {
      html +=
        '<br><span class="vocab">Press "v" for vocabulary notes.</span><br>';
      addFinalBr = true;
    }
  ...
  // Add Cantonese entries if available
  if (
    result.cantonese &&
    result.cantonese.data &&
    result.cantonese.data.length > 0
  ) {
    html += '<br><hr class="cantonese-separator"><br>';
    html += '<span class="cantonese-header">Cantonese</span><br><br>';

    for (let i = 0; i < result.cantonese.data.length; ++i) {
      // Match Cantonese format: traditional simplified [pinyin] {jyutping} /definition/
      // Note: Some entries have "# adapted from cc-cedict" at the end
      let cantEntry = result.cantonese.data[i][0].match(
        /^([^\s]+?)\s+([^\s]+?)\s+\[(.*?)\]?\s*\{(.*?)\}\s*\/(.+?)\/?\s*(?:#.*)?$/
      );

      // Hanzi (reuse from Mandarin entry - same characters)
      hanziClass = "w-hanzi";
      if (config.fontSize === "small") {
        hanziClass += "-small";
      }
      ...
      if (config.simpTrad === "auto") {
        let word = result.cantonese.data[i][1];
        html += '<span class="' + hanziClass + '">' + word + "</span>&nbsp;";
      } else {
        html +=
          '<span class="' + hanziClass + '">' + cantEntry[2] + "</span>&nbsp;";
        if (cantEntry[1] !== cantEntry[2]) {
          html +=
            '<span class="' +
            hanziClass +
            '">' +
      ...
      let jyutping = result.cantonese.data[i][2];
      if (jyutping) {
        let j = jyutpingFormat(jyutping, showToneColors, jyutpingClass);
        html += j[0];
      }
      ...
      let cantTranslation = cantEntry[5].replace(/\//g, " ◆ ");
      html +=
        '<br><span class="' + defClass + '">' + cantTranslation + "</span><br>";

      // Add spacing between entries
      if (i < result.cantonese.data.length - 1) {
        html += "<br>";
      }
    }

    if (result.cantonese.more) {
      html += "&hellip;<br/>";
    }
  }
...
function jyutpingFormat(jyutping, showToneColors, jyutpingClass) {
  // Format Jyutping similar to Pinyin
  // Jyutping uses tone numbers at the end (1-6)
  let text = "";
  let html = "";
  let syllables = jyutping.split(/[\s]+/);
  ...
  for (let i = 0; i < syllables.length; i++) {
    ...
    let toneMatch = syllable.match(/^(.+?)([1-6])$/);
    if (toneMatch) {
      let base = toneMatch[1];
      let tone = toneMatch[2];

      if (showToneColors) {
        html += '<span class="' + jyutpingClass + " tone" + tone + '">';
      } else {
      ...
      // No tone number, use neutral tone
      if (showToneColors) {
        html += '<span class="' + jyutpingClass + ' tone5">';
      } else {
        html += '<span class="' + jyutpingClass + '">';
      }
      html += syllable;
...
function pinyinAndZhuyin(syllables, showToneColors, pinyinClass) {
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    zhuyin +=
      ...
      zhuyinClass +
      '">' +
      globalThis.numericPinyin2Zhuyin(syllable) +
      "</span>";
  }
